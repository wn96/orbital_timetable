...
def check_invalid(url):
    return expand_url(
        url) == "http://modsn.us" or expand_url(url) == "http://modsn.us/"
...
/start - This bot allow groups to add their timetable to the chat. The chatbot will then inform groups of timings where members are mutually available.", chat)
                ...
                if text == "/add":
                    ...
                    if item in items:
                        ...
                        send_message(
                            "Timetable has been successfully added!", chat)
                ...
                if not items:
                    send_message(
                        "There are no timetables added to the chat yet", chat)
                else:
                    for i in items:
                        stuff += str(counter) + ". " + i + "\n"
                        counter += 1
                ...
                if text == "/del":
                    send_message(
                        "Please type '/del <NUSMODS link>' to delete the timetable.", chat)
                ...
                week = text[6:]
                if text == "/week":
                    if isinstance(constants.WEEK_TODAY, int):
                        week = constants.WEEK_TODAY
                    elif isinstance(constants.WEEK_TODAY, str):
                        send_message(
                            "There is no school during " +
                            constants.WEEK_TODAY +
                            "!",
                            chat)
                        continue
                ...
                if week <= 13 and week >= 0:  # 0 <= week <= 13
                    ...
                    send_message(
                        "Week " +
                        str(week) +
                        " does not exist! Please enter an integer between 1 and 13, inclusive.",
                        chat)
        ...
        send_message(
            "An error has occured! Please try again. If the problem persist, please email support at weineng.a@gmail.com!",
            chat)
